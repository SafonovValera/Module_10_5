...
# Далее, если есть свободный стол, то сажать гостя за стол (назначать столу guest), запускать поток гостя и выводить на
...
# Далее запустить поток этого гостя (start).
    ...
    def discuss_guests(self):
        while not self.queue.empty() or self.guest_sit():
            for table in self.tables:
                ...
                if not self.queue.empty() and table.guest is None:
                    ...
                    next_guest.start()
                    print(f'{next_guest.name} вышел(вышла) из очереди и сел(-а) за стол номер {table.number}')
